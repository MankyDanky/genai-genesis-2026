...
```
User prompt → ChatPanel → POST /api/chat (with currentCode) → Codex Sonnet 4.6 (streamed)
...
1. **API route** (`src/app/api/chat/route.ts`) — Streams responses from Codex via Vercel AI SDK v6. Has one tool: `update_sandbox` (takes a `code` string). Uses `stopWhen: stepCountIs(2)` to limit tool-use rounds. The `currentCode` from the request body is passed to the system prompt builder.
